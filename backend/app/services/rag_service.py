...
from app.models.schemas import Document, LLMConfig, QueryRequest
    ...
    def on_llm_start(
        ...
        try:
            ...
            if self.request_count > self.max_requests_per_minute * 0.8:
                logger.warning(
                    f"Approaching request rate limit: {self.request_count}/{self.max_requests_per_minute}"
                )
            ...
            if isinstance(prompts[0], str) and settings.GOOGLE_API_KEY:
                try:
                    ...
                    logger.info(
                        f"Input tokens: {token_count} (Total this minute: {self.total_tokens_per_minute})"
                    )

                    # Check if approaching limit
                    if self.total_tokens_per_minute > self.max_tokens_per_minute * 0.8:
                        logger.warning(
                            f"Approaching token rate limit: {self.total_tokens_per_minute}/{self.max_tokens_per_minute}"
                        )
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    self.total_tokens_per_minute += token_count
                    logger.info(
                        f"Output tokens: {token_count} (Total this minute: {self.total_tokens_per_minute})"
                    )
                    logger.info(
                        f"Total tokens for this request: {self.input_tokens + self.output_tokens}"
                    )
                except Exception as token_error:
...
class EnhancedRAGService:
    ...
    async def process_document(self, document: Document, file_path: str) -> bool:
        ...
        try:
            ...
            processed_chunks = await self.document_processor.process_document(
                document, file_path
            )

            if not processed_chunks:
                logger.warning(
                    f"No chunks extracted from document: {document.filename}"
                )
                return False
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        """You are a helpful assistant that extracts file filtering information from queries.
            ...
            if match:
                ...
                try:
                    ...
                    # Clean up metadata
                    return {
                        k: v for k, v in metadata.items() if v and v.lower() != "null"
                    }
    ...
    async def _perform_hybrid_search(
        ...
        query: str,
        k: int = 5,
        session_document_filter: Optional[List[str]] = None,
    ) -> List[Dict]:
        ...
        try:
            ...
            if session_document_filter:
                ...
                try:
                    ...
                    if metadata_filters:
                        logger.info(
                            f"Extracted query metadata filters: {metadata_filters}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Failed to extract query metadata, proceeding without: {str(e)}"
                    )
                    metadata_filters = None
            ...
            logger.info(
                f"Semantic search completed in {time.time() - start_time:.2f}s with {len(search_results)} results"
            )
    ...
    def _re_rank_results(self, results: List[Dict], query: str) -> List[Dict]:
        ...
        for result in results:
            ...
            adjusted_score = (
                result["score"] + (keyword_matches * 0.05) + (first_para_matches * 0.1)
            )
    ...
    async def _get_session_document_filter(
        ...
        try:
            ...
            async with AsyncSessionLocal() as db_session:
                ...
                chat_session = await ChatService.get_session_by_uuid(
                    db_session, session_uuid
                )
                if not chat_session:
                ...
                documents = await ChatService.get_session_documents(
                ...
                logger.info(
                    f"Session {session_uuid} has {len(document_uuids)} associated documents"
                )
    ...
    async def generate_response(
        self, query: QueryRequest, session_uuid: Optional[str] = None
        ...
        try:
            ...
            if provider != "gemini":
                logger.warning(
                    f"Requested provider {provider} not available, using Gemini"
                )
                provider = "gemini"
            ...
            if provider == "gemini":
                ...
                if not gemini_limiter.can_make_call():
                    ...
                    logger.warning(
                        f"Gemini rate limit reached, waiting {wait_time:.2f}s"
                    )
                ...
                if hasattr(self.gemini_model, "callbacks"):
                    for callback in self.gemini_model.callbacks:
                        if isinstance(callback, EnhancedTokenDebugHandler):
                            if (
                                callback.total_tokens_per_minute
                                > callback.max_tokens_per_minute * 0.8
                                or callback.request_count
                                > callback.max_requests_per_minute * 0.8
                            ):
                                logger.warning(
                                    "Approaching Gemini token/request limits, waiting before proceeding"
                                )
                                yield self.format_sse(
                                    {
                                        "type": "warning",
            ...
            if session_uuid:
                session_document_filter = await self._get_session_document_filter(
                    session_uuid
                )
            ...
            for i, result in enumerate(search_results, 1):
                ...
                if total_context_length + len(chunk_text) > max_context_length:
                    logger.info(
                        f"Context limit reached, using {len(context_parts)} out of {len(search_results)} documents"
                    )
                    break
            ...
            try:
                ...
                try:
                    async for chunk in chain.astream(query.query):
                        ...
                        if len(sentences) > 1:  # We have at least one complete sentence
                            complete_sentences = sentences[
                                :-1
                            ]  # All but the last (possibly incomplete) sentence
                            buffer = sentences[
                                -1
                    ...
                    if (
                        ...
                        logger.warning(
                            f"Rate limit error during streaming: {stream_error}"
                        )
                        yield self.format_sse(
                            {
                                "type": "error",
                ...
                # Signal completion
                yield self.format_sse(
                    {"type": "done", "content": "", "provider": provider}
                )
            ...
            yield self.format_sse(
                {"type": "error", "content": f"Error in RAG pipeline: {str(e)}"}
            )
